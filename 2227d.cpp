...
void solve()
{
    ll n,i,j,c=0,f=0,k=0,l=0,a1=0,a2=0,a3=0;
    ...
    if(c>=0 && f<=2*n-1)
    {
        while(c>=0 && f<=2*n-1 && a[c]==a[f])
        {
            c--;
            f++;
        }
        a1=max(mex(vector<ll>(a.begin()+c+1,a.begin()+f)),1ll);
    }
    ...
    if(c>=0 && f<=2*n-1)
    {
        ...
        a2=max(mex(vector<ll>(a.begin()+c+1,a.begin()+f)),1LL);
    }
    c=i,f=j;
    while(i<=j && a[i]==a[j])
    {
    ...
    else
    {
        ...
        {
            a3=max(mex(vector<ll>(a.begin()+c+1,a.begin()+f)),1ll);
        }
    }
